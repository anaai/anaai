...
describe("StyleNFT", () => {
  ...
  describe("mintNFT", () => {
    ...
    it("Emits TokenMinted event", async () => {
      ...
      const tx = await this.contract.mintNFT(owner, user1, uri, {from: owner});

      expectEvent(
        tx,
        "TokenMinted",
        {recipient: owner, payer: user1, tokenId, tokenURI: uri}
      );
    });

    it("Doesn't mint a token when you are not the owner", async () => {
      await expectRevert(
        this.contract.mintNFT(user1, user1, uri, {from: user1}),
