...
@patch("service.send_task")
def test_cartoonify_response(cartoonify_mock):
  task_id = 555333
  cartoonify_mock.return_value.id = task_id
...
def test_cartoonify_task_invocation(cartoonify_mock):
  ...
  payer = "payer"
  price = 0
  cartoonify_mock.return_value.id = 555333

  response = client.post("/cartoonify", json={"image_url": url, "image_name": name, "payer": payer})

  cartoonify_mock.assert_called_with("tasks.cartoonify", [ANY, payer, price, url, name])
